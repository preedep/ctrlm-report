...
test.describe('CTM Migration Tab', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('#mig-table tbody tr', { timeout: 10_000 });
  });
  ...
  test.describe('Migration table', () => {
    test('shows all 7 plan items', async ({ page }) => {
      await expect(page.locator('#mig-table tbody tr')).toHaveCount(D.PLAN_TOTAL);
    });

    test('Done badge visible for completed job', async ({ page }) => {
      const row = page.locator('#mig-table tbody tr').filter({
        hasText: D.DONE_JOB,
      });
  ...
  test.describe('Search', () => {
    test('searching by job name filters table', async ({ page }) => {
      ...
      await expect(page.locator('#mig-table tbody tr')).toHaveCount(1);
    });

    test('searching by SR number filters table', async ({ page }) => {
      await page.locator('#mig-search').fill('SR-55064');
      await page.waitForTimeout(400);
      const count = await page.locator('#mig-table tbody tr').count();
    ...
    test('non-matching search shows empty state', async ({ page }) => {
      ...
      await page.waitForTimeout(400);
      await expect(page.locator('#mig-table tbody tr')).toHaveCount(0);
    });
  });
  ...
  test.describe('Status filter', () => {
    test('filtering by "Done" shows only done rows', async ({ page }) => {
      ...
      await expect(page.locator('#mig-table tbody tr')).toHaveCount(D.PLAN_DONE);
    });
    ...
    test('filtering by "In Progress" shows only in-progress rows', async ({ page }) => {
      ...
      await page.waitForTimeout(200);
      await expect(page.locator('#mig-table tbody tr')).toHaveCount(D.PLAN_IN_PROGRESS);
    });
  });
  ...
  test.describe('Cell navigation', () => {
    test('clicking Job Name cell navigates to Jobs tab filtered by that job', async ({ page }) => {
      const row = page.locator('#mig-table tbody tr').filter({ hasText: D.DONE_JOB });
  ...
  test('Export CSV button is visible', async ({ page }) => {
    await expect(
      page.locator('#mig-export-btn, button:has-text("Export CSV")'),
    ).toBeVisible();
  });
});
