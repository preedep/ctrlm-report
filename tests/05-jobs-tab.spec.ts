...
test.describe('Jobs Tab', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('#jobs-table tbody tr', { timeout: 10_000 });
  });
  ...
  test('table has Migration column', async ({ page }) => {
    const headers = page.locator('#jobs-table thead th');
    const labels  = await headers.allTextContents();
    expect(labels.some(h => /migration/i.test(h))).toBe(true);
  });

  test('Export CSV button is visible', async ({ page }) => {
    await expect(page.locator('#export-csv-btn, button:has-text("Export CSV")')).toBeVisible();
  });
  ...
  test.describe('Filters', () => {
    ...
    test('selecting a domain filters the table', async ({ page }) => {
      ...
      const count = await page.locator('#jobs-table tbody tr').count();
      expect(count).toBeGreaterThan(0);
  ...
  test.describe('Column sorting', () => {
    test('clicking Job Name header sorts ascending', async ({ page }) => {
      const jobNameHeader = page.locator('#jobs-table thead th').first();
      await jobNameHeader.click();
      await page.waitForTimeout(200);
      const firstCell = page.locator('#jobs-table tbody tr').first()
        .locator('td.job-name');
    ...
    test('clicking Job Name header twice sorts descending', async ({ page }) => {
      const jobNameHeader = page.locator('#jobs-table thead th').first();
      await jobNameHeader.click();
      const asc = await page.locator('#jobs-table tbody tr td.job-name').first().textContent();
      await jobNameHeader.click();
      const desc = await page.locator('#jobs-table tbody tr td.job-name').first().textContent();
  ...
  test.describe('Pagination', () => {
    ...
    test('next page button loads next page', async ({ page }) => {
      const firstJobBefore = await page.locator('#jobs-table tbody tr td.job-name')
        .first().textContent();
      await page.locator('#next-page-btn').click();
      await page.waitForTimeout(200);
      const firstJobAfter = await page.locator('#jobs-table tbody tr td.job-name')
        .first().textContent();
  ...
  test.describe('Migration column', () => {
    test('Done badge appears for migrated jobs', async ({ page }) => {
      ...
      await page.waitForTimeout(400);
      const row = page.locator('#jobs-table tbody tr').first();
